<script>
    ...
    import { neighbours, bombCount, tilesPerRow, totalTiles, gameStarted, countNearbyBombs } from "./stores.js";
...
<div id="MinesField"
     style="grid-template-columns: repeat({$tilesPerRow}, auto)">
    {#each bombPlacements as tile, tileIndex}
        <Cells
                ...
                {tileIndex}
                {bombsDetonated}
                {tileAtIndexIsClicked}
                {tileAtIndexIsFlagged}
                {tileAtIndexIsMarked}
                on:updateTimer
                on:layBombs|once="{layBombs}"
                on:gameOver|once="{gameOver}"
                on:revealNeighbours="{revealNeighbours}"
        />
    {/each}
</div>
